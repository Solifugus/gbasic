...
./tests/run_xlsx.sh         # xlsx Stage 1 (docs/xlsx_design.md): ZIP container over zlib + the PART TREE + read-only sparse cells. The design's load-bearing claim is that the reader DISCARDS NOTHING -- round-trip write is the destination, and a reader that quietly drops a part looks healthy until write silently produces a lesser file. Tiers: GOLDEN (examples/xlsx_read_test.bas -- shared strings incl. an XML entity and non-ASCII, a sparse sheet with no row 4 and no column C, a formula cell keeping BOTH <f> and Excel's cached <v>, a boolean, an Excel error as its own kind, style indices preserved because only the number format distinguishes a date from 45000, and a second sheet resolved via r:id since part filenames follow creation order not tab order); RETENTION (every entry in the ZIP's own central directory must appear in xlsx.parts -- compared against `unzip -Z1`, never a hardcoded list, so growing the fixture cannot leave the check narrower than the file -- plus the unmodelled vendor part returned byte-identical); NEGATIVE (not-a-ZIP, truncated, missing file); VALGRIND. Skips cleanly when built without zlib or libxml2. No python3 in the test path -- the fixture is committed and tools/make_xlsx_fixture.py is only how it was authored
