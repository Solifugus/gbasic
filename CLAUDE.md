...
./tests/run_studio.sh       # STU-0..STU-5A Studio (+ STU-STORE: studio_store's read path over try_decode -- three states unchanged, corrupt files now carrying the parser's reason with line/column, every store shape round-tripping, a 115KB/240-record results index opening in under 5s where the deleted pure-gBASIC pre-validator took 92s, and an assertion that studio_store references no validator at all) (stdlib/studio*.bas): STU-0 backbone (project/session/settings model + versioned atomic persistence + startup/shutdown), STU-1 navigation (workspace registry + filesystem project browser), STU-2 documents (headless document manager: open/dedupe/dirty/save/close/external-change/restore + editor-tab shell), STU-3 execution sections (studio_sections over source_outline: structural derivation, Studio-owned stable ids, anchors+whitespace-folding fingerprints, bidirectionally-unique 4-tier reattachment, ambiguous/stale flagging, last-known-good on invalid source, byte-offset and line/column cursor resolution, additive ws.sections persistence). Headless + GI-independent; goldens path-free; STU-0 empty-startup/save-restore/corrupt/future-version/atomic-stress + STU-1 workspace-lifecycle/multi-project/nav-persistence/missing-project/browser/tree-refresh/registry + STU-2 lifecycle/save/savefail/close×3/external/restore/missing/browser + STU-3 derive/cursor/cursor_pos/prog/insert_blank/internal/rename/sibling/duplicate/delete/invalid/persist/store/compat/multidoc/unicode/repeated, plus valgrind-clean memory probes (SKIPs valgrind tier if absent) and an optional STU-3 display tier (sections_gui: GTK shell + live editor cursor; SKIPs without GTK 4/display) PLUS STU-4 execution sessions (studio_session: replay-first runs of sections 1..N in a child `./gbasic --line-buffered --json-diagnostics <byte-prefix>`, 8-state machine idle/materializing/running/stopping/unresponsive/finished/failed/refused with every transition golden-tested, append-only prefix materialization from in-memory content, error attribution to target/prefix/outside via STU-3 ranges, polite stop vs explicit force escalation, restart-mid-run, three refusals, scratch cleanup asserted after every case) + a valgrind tier + an optional STU-4 display tier (sessions_gui: GTK shell + gi.timeout-driven run; SKIPs without GTK 4/display) PLUS STU-4B materialization completeness (declaration hoisting of post-target top-level function/modifier/library decls — only when the prefix executes a program block, the exact condition under which the interpreter itself pre-registers them; per-run nonce boundary marker injected as ONE line at the N-1/N boundary with a single -1 offset, because gBASIC has no statement separator; per-stream split_out exact/marked/combined + split_err exact/unavailable with ambiguity falling back rather than guessing; one shared position map translating materialized lines back to document lines across marker/generated/hoisted, probed directly by sessions_map; sessions launch with --line-buffered so output arrives mid-run — hoist/hoist_before/hoist_order/hoist_err/hoist_target/hoist_inert/split/split_nonce/split_die/split_stderr/map/stream) PLUS STU-5A persistent results (studio_results: a durable per-run record -- section id, PLUS the fingerprint of the section content AS RUN, timing, exit/signal, per-stream captures with their split verdicts, attributed diagnostics, and refusal codes). The store is SEPARATE from the workspace record (results move ~155KB in one run; STU-3's anchors are small and bounded): `<home>/results/<key>.json`, one file per DOCUMENT keyed by PATH not doc_id (doc-N is minted per launch from open order so it is not stable across restarts, and a result history is not self-healing the way STU-3's anchor cache is), strict JSON through studio_store's atomic_replace discipline. Retention 20 per SECTION evicted on write; captures capped at 64KB keeping the HEAD, truncation recorded BOTH in a `truncated` field and as a notice inside the stored text. `standing_of` compares a stored result against the CURRENT sections on every read and is never stored -- current/stale-content/section-ambiguous/section-gone/source-invalid. A pre-STU-5A home needs no migration (no results dir = empty store); corrupt and future-schema stores are reported, not misread. 12 headless cases (persist/history/fingerprint/orphan/refused/signal/truncate/evict/concurrent/compat/store/view) + an on-disk store-separation assert + a disk-size INFO line + a valgrind tier + an optional display tier (results_gui: the pane rendered before a run, after the result is re-read from disk, and after an edit where the stale-content mark must appear; SKIPs without GTK 4/display)
