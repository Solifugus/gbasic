...
./tests/run_studio.sh       # STU-0/STU-1/STU-2 Studio (stdlib/studio*.bas): STU-0 backbone (project/session/settings model + versioned atomic persistence + startup/shutdown), STU-1 navigation (workspace registry + filesystem project browser), STU-2 documents (headless document manager: open/dedupe/dirty/save/close/external-change/restore + editor-tab shell). Headless + GI-independent; goldens path-free; STU-0 empty-startup/save-restore/corrupt/future-version/atomic-stress + STU-1 workspace-lifecycle/multi-project/nav-persistence/missing-project/browser/tree-refresh/registry + STU-2 lifecycle/save/savefail/close×3/external/restore/missing/browser, plus valgrind-clean memory probes (SKIPs valgrind tier if absent)
