...
./tests/run_studio.sh       # STU-0/STU-1 Studio (stdlib/studio*.bas): STU-0 backbone (project/session/settings model + versioned atomic persistence + startup/shutdown) and STU-1 navigation (workspace registry + filesystem project browser). Headless + GI-independent; goldens path-free; STU-0 empty-startup/save-restore/corrupt/future-version/atomic-stress + STU-1 workspace-lifecycle/multi-project/nav-persistence/missing-project/browser/tree-refresh/registry, plus valgrind-clean 50-cycle memory probes (SKIPs valgrind tier if absent)
