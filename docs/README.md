...
| [automation_reasoning_design.md](automation_reasoning_design.md) | **Proposal** | Business Automation Reasoning — making programmable the reasoning layer BI stops short of: observe conditions, detect meaningful change, investigate why, evaluate alternatives, decide under uncertainty, act within defined authority, escalate what needs judgement, and measure whether the last action worked. Three proposed libraries — `insight`, `decision`, `automation` — over a shared business-reasoning model, with provenance, materiality/confidence/risk/authority, and a cookbook-first development route. **Nothing here is built.** §28-§30 record what Recipe 1 measured, including that §11's decomposition needs a null model to be buildable at all, and add the refusals the document originally lacked. |
