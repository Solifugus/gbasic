...
## 11. A dashboard axis: categories, currency, and clickable marks

Three things a real dashboard needs that a chart of numbers does not.

**A categorical x.** "Revenue by month" has a period *label* on its x axis, not
a number — and `line`, `area` and `scatter` place those at the same band
centres `bar` uses, so a line and a bar of the same frame line up column for
column. A column mixing categories and numbers is refused: there is no axis
that is honest about both.

**An axis that agrees with the table beside it.** `y_prefix` / `y_suffix` /
`y_decimals` / `y_scale` (and the `x_` equivalents). `y_scale` is **display
only and moves no geometry** — it is what makes a fraction readable as a
percentage without charting a different number. An explicit `y_decimals` means
*fixed* places, since a requested `2` that renders `$4,000` is a setting that
appears not to work.

**Marks that say what they are.** `mark_keys: true` puts `data-series` and
`data-key` on every data mark, so a browser can answer "which bar was clicked"
without matching on DOM order. A legend swatch is not a data mark and carries
nothing — DOM order is exactly what stops being reliable once a legend adds
rects. Off by default: the core stays static and JS-free, and the JavaScript
stays yours.

<!--CODE:11_dashboard_axes-->
...
<!--OUT:11_dashboard_axes-->

```
line points: 4
bar rects:   4
month labels on the line: 4
money axis: true
percent axis: true
keyed marks: 4
first bar:   data-series="revenue" data-key="2026-01"
```
