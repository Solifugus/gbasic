...
| 7 | `r["typo"]` compared to a value | `unknown`, which compares not-equal silently | **BUILT, MEASURED, REVERTED 2026-08-23.** Narrowed twice and still wrong. Firing on any computed `unknown` hit `file_type(p) = "folder"` and `env(V) = "1"`; narrowed to INDEX reads it was silent across gBASIC but fired twice in Studio, **correctly both times** — `shell.marked[doc] != rev` and `stated(cfg)[tier] = v`, where absent-therefore-not-equal is the intended reading. Nothing separates that from a typo without value provenance or a statement of intent. **2026-09-07: this trap cost an AUTHENTICATION BYPASS** in `examples/steward/steward.bas` — `if req.headers["x-user"] = ""` never fired, because a missing header is `unknown` and `unknown = ""` is false, so a request carrying no identity was answered 202 and a run was created for nobody. The measurement above still stands and the row stays REVERTED; what it argues for is a NARROWER rule than the ones tried — an `unknown` compared specifically to the EMPTY STRING, which is the shape of "is this absent" and is always wrong — and that has not been measured. Recorded here so the next attempt starts from the narrow version rather than re-deriving the two that failed |
| 9 | ~~a write to a `for each` element that nothing reads~~ **SHIPPED 2026-09-08** | silent: the loop variable is a COPY, so `item.x = 1` is discarded when the iteration ends | warns once per loop at its LAST dead write (`2107`, source `discarded write`). **THE DISCRIMINATOR IS NOT THE ASSIGNMENT, IT IS WHETHER THE VALUE IS READ AGAIN**, and that distinction is the whole design: writing to the element is LEGITIMATE and ordinary — `row = normalise(row)` then `print row.name`, or `row.label = tag(row)` then `send(row)` — so the syntactic rule "you assigned to a loop variable" flags correct code, which is precisely the shape that got row 6 built, measured at 287 false positives and reverted. A write nothing reads cannot affect anything under any semantics, so only that is reported. **MEASURED BEFORE BUILDING**: 3 writes to a for-each variable exist across stdlib, examples and tests; the rule fires on exactly 1, the fixture that demonstrates the discard; and the correct write-back idiom stays silent WITHOUT being special-cased, because `list[i] = item` reads `item`. Re-measured after building by running every example and test: **0 false positives**. The analysis is deliberately CONSERVATIVE and errs towards silence — it compares SOURCE POSITIONS rather than doing dataflow, so a read on any later line suppresses it, a `goto`/`gosub`/label in the body suppresses it (source order stops implying execution order), and an unmodelled construct suppresses it. A missed dead write costs nothing; a false one costs the channel's credibility. Affordable only because `for each item, i in list` shipped the same day, so the message names a remedy that exists rather than describing one |
| 8 | ~~a top-level statement beside a `program` block~~ **SHIPPED 2026-09-07** | silent dead code: the block is what runs, and the top level is read only for declarations | warns once at the first such statement (`2106`, source `dead code`). **Measured before shipping: of 420 files in this tree with a program block, FOUR have a top-level statement, and one of those is `tests/native_platform/plat_guard_prereg_child.bas`, which exists to assert exactly this** — so the diagnostic costs almost nothing. It was written after the trap was hit THREE TIMES IN ONE DAY building the AI stack: a toolset assembled at the top level was simply absent, and a `watch(inbox.messages)` up there registered NOTHING, so every worker reply went undelivered and every conversation hung with the symptom pointing nowhere near the cause. A warning rather than a refusal because the program still runs exactly as it did; `watch` in particular cannot simply be hoisted with the declarations, because a watcher FIRES on registration and would run its body before anything it reads was assigned |
