...
program demo(args)
    ...
    print("kripp nominal " + string(round(krippendorff_alpha(data, "nominal").alpha, 6)))
    print("kripp interval " + string(round(krippendorff_alpha(data, "interval").alpha, 6)))
    print("kripp ordinal " + string(round(krippendorff_alpha(data, "ordinal").alpha, 6)))

    ' domain guards return unknown
    print("guard_cronbach " + string(cronbach_alpha([ [1, 2] ])))
    print("guard_kappa " + string(cohens_kappa([1, 2], [1])))
    print("guard_icc " + string(icc([ [1, 2] ])))
end program
