...
program demo(args)
    ...
    m = stats.ols(y, [x1, d.columns[0], d.columns[1]])
    ...
    ' R2 0.9525.
    xB = stats.interaction(x1, d.columns[0])
    xC = stats.interaction(x1, d.columns[1])
    mi = stats.ols(y, [x1, d.columns[0], d.columns[1], xB, xC])
    ...
    print("guard_dummy " + string(stats.dummy_code(["A", "A"])))
    print("guard_intr " + string(stats.interaction([1, 2], [1])))
end program
