...
program demo(args)
    ...
    r = stats.simple_returns(prices)
    lr = stats.log_returns(prices)
    print("returns simple1 " + string(round(r[0], 4)) + " log1 " + string(round(lr[0], 4)) + " cum " + string(round(stats.cumulative_return(r), 4)))

    ' ---- Performance & risk metrics ----
    print("perf sharpe " + string(round(stats.sharpe_ratio(r, 0.0, 252), 3)) + " sortino " + string(round(stats.sortino_ratio(r, 0.0, 252), 3)))
    md = stats.max_drawdown(prices)
    print("risk maxdd " + string(round(md.max_drawdown, 4)) + " var95 " + string(round(stats.value_at_risk(r, 0.05, "historical"), 4)) + " cvar " + string(round(stats.cvar(r, 0.25), 4)))
    ...
    bp = stats.breusch_pagan(fit.residuals, [xx])
    ...
    a1 = stats.adf_test(lvl, 1, "c")
    dlvl = stats.diff(lvl, 1)
    a2 = stats.adf_test(dlvl, 1, "c")
    ...
    arch = stats.arch_lm(ret, 2)
    g = stats.garch_fit(ret)
    ...
    lb = stats.ljung_box(fit.residuals, 4)
    print("resid ljung_box p " + string(round(lb.p_value, 3)) + " dw " + string(round(stats.durbin_watson(fit.residuals), 3)))
    ...
    ev = stats.abnormal_returns(ast_r, mkt_r, { event: 150, pre: 1, post: 1, estimation: 120 })
