...
program demo(args)
    ...
    print("rows " + string(mat_rows(a)))
    print("cols " + string(mat_cols(a)))

    ' Transpose.
    t = mat_transpose([[1, 2, 3], [4, 5, 6]])
    print("t00 " + string(t[0][0]))
    print("t21 " + string(t[2][1]))

    ' Product (2x2)(2x2).
    prod = mat_mul([[1, 2], [3, 4]], [[5, 6], [7, 8]])
    ...
    ' Dot product.
    print("dot " + string(vec_dot([1, 2, 3], [4, 5, 6])))

    ' Singular matrix has no inverse.
    print("sing " + string(mat_inverse([[1, 2], [2, 4]])))
end program
