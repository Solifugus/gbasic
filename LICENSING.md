...
- `stdlib/`: `accounting` `agent` `ari` `ari_discover` `automation` `chart` `credit` `crypto` `datagrid` `dates` `decision` `discovery` `deposits` `estate` `fake` `filetree` `finance` `finio` `finio_all` `finio_bai2` `finio_camt` `finio_iso20022` `finio_nacha` `finio_ofx` `finio_pain001` `finio_registry` `finio_watch` `lending`
