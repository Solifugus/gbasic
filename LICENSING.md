...
- `stdlib/`: `accounting` `agent` `ari` `automation` `chart` `credit` `crypto` `datagrid` `dates` `decision` `discovery` `deposits` `estate` `fake` `filetree` `finance` `finio` `finio_all` `finio_bai2` `finio_camt` `finio_nacha` `finio_ofx` `finio_registry` `finio_watch` `lending`
