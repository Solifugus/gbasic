...
topics = pg.query(db, "select t.title, c.slug as category_slug from gbasic_site_topics t join gbasic_site_categories c on c.id = t.category_id where t.hidden = false order by t.id")
print(len(topics))
print(topics[0].category_slug)
print(topics[0].title)

posts = pg.query(db, "select body from gbasic_site_posts where hidden = false order by id")
print(len(posts))
print(posts[0].body)
