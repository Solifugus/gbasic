...
gr = datagrid.add_column(gr, { title: "Name", field: "name" })
gr = datagrid.add_column(gr, { title: "Balance", field: "balance" })
...
ga = datagrid.add_column(ga, { title: "Name", index: 0 })
ga = datagrid.add_column(ga, { title: "Bal", index: 1 })
...
gc = datagrid.add_column(gc, { title: "Name", field: "name" })
src[0].name = "MUTATED"
print "snapshot (source mutation ignored): " + datagrid.cell(gc, 0, 0)
gc = datagrid.set_rows(gc, [ { name: "replaced" }, { name: "second" } ])
...
gv = datagrid.set_count(gv, 500000)
