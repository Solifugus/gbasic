...
DOC=docs/money_cookbook.md
DIR=examples/money_cookbook
...
if [ ! -f "$DOC" ]; then
    note_fail "run_money_cookbook: $DOC not found"
    exit 1
fi

if [ ! -x ./gbasic ]; then
    make >/dev/null 2>&1 || { note_fail "run_money_cookbook: build failed"; exit 1; }
fi
...
# `examples/money_cookbook/*.bas`, and a variable would read as unwired.
recipes=$(ls examples/money_cookbook/*.bas 2>/dev/null | sed 's|.*/||; s|\.bas$||' | sort)
if [ -z "$recipes" ]; then
    note_fail "run_money_cookbook: no recipes in $DIR"
    exit 1
fi
...
for r in $recipes; do
    ...
    if [ ! -s "$blk" ]; then
        note_fail "$DOC (no code block for $r -- add <!--CODE:$r--> and run tools/sync_money_cookbook.sh)"
    elif ! diff -u "$bas" "$blk" >/dev/null 2>&1; then
        note_fail "$DOC code block for $r differs from $bas -- run tools/sync_money_cookbook.sh"
    ...
    if [ ! -s "$blk" ]; then
        ...
        note_fail "$DOC output block for $r differs from $out -- run tools/sync_money_cookbook.sh"
...
printf '\nrun_money_cookbook: PASS=%d SKIP=%d\n' "$pass" "$skip"
exit "$status"
