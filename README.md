...
GUI — and forty-two pure-gBASIC libraries covering double-entry accounting, loan
...
**114 test suites** gate every change, goldens are byte-exact, and the claims in
...
Forty-two pure-gBASIC libraries in `stdlib/`. Each bullet says what backs it,
...
- **calling a model, and letting it call you back** (`llm`, `tools`, `agent`) — chat
  ...
  [tools](docs/tools_design.md) · [agent](docs/agent_design.md) · [llm](docs/llm_design.md)
...
**Use `run_all.sh` rather than naming suites.** It discovers all 114 suites by
