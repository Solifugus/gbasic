...
GUI — and forty-one pure-gBASIC libraries covering double-entry accounting, loan
...
**111 test suites** gate every change, goldens are byte-exact, and the claims in
...
Forty-one pure-gBASIC libraries in `stdlib/`. Each bullet says what backs it,
...
- **calling a model, and letting it call you back** (`llm`, `tools`) — chat
  ...
  bodies run in a pre-spawned worker pool rather than on the event loop.
  [Design](docs/tools_design.md)
...
**Use `run_all.sh` rather than naming suites.** It discovers all 111 suites by
